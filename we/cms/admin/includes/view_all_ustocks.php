...
while($row = mysqli_fetch_assoc($select_stocks)){
    ...
    $query2 = "SELECT * FROM properties WHERE symbol = '{$symbol}' ";
    $select_ustocks = mysqli_query($connection,$query2);
    
    while ($row2 = mysqli_fetch_assoc($select_ustocks)) {
...
echo "<td class='column100 column16' data-column='column16'><a onClick=\"javascript: return confirm('Are you sure you want to delete?'); \" href='./admin_uport.php?delete={$symbol}'>Delete</a></td>";
echo "</tr>";
...
?>
			...
			<div class="wrapper">
			    <button type="button" class="btn btn-info" onclick="window.location.href = './admin_uport.php?source=add_stock';">Add stocks</button>
...
if(isset($_GET['delete'])){
    ...
    $query = "DELETE FROM Portfolio WHERE symbol = '{$the_symbol}' ";
